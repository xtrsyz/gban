...
local function getBottomY()
	local bottomY = maxOption < getStyleProperty('maxOptionCountOnScreen') and maxOption or getStyleProperty('maxOptionCountOnScreen')
	return bottomY +1
end
...
function WarMenu.ToolTip(text, width, flipHorizontal)
	...
	local y = getStyleProperty('y') + titleHeight + (buttonHeight * getBottomY()) + height / 2

	drawRect(x, y, width, height, getStyleProperty('titleBackgroundColor'))
